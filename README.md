...
### 🗄️ MariaDB / RDBMS Fundamentals
> 정확한 날짜 기록이 없어 범위로 표기 (추정)
- [x] 2026.09.05 ~ 09.11 (추정) : Docker + MariaDB 환경 구축, DDL(CREATE/ALTER/DROP/TRUNCATE)
- [x] 2026.09.05 ~ 09.11 (추정) : DML(INSERT/SELECT/UPDATE/DELETE), 집계함수, GROUP BY/HAVING
- [x] 2026.09.05 ~ 09.11 (추정) : JOIN(INNER/LEFT/RIGHT), 서브쿼리, 상관 서브쿼리
- [x] 2026.09.05 ~ 09.11 (추정) : 정규화(1NF~3NF), 1:1 / 1:N / N:M 관계 설계, 복합 PK, 인덱스
- [x] 2026.09.05 ~ 09.11 (추정) : 트랜잭션(TCL), 권한 관리(DCL) 개념
...
- [x] 2026.09.15~16 : RBAC 권한 시스템(roles 테이블), 관리자 페이지, 마이페이지

### 👷 Worker Management App (신규 클론 프로젝트)
